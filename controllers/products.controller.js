...
const findAll = async () => {
    return query("SELECT ProductID, Name, Price FROM products");
};

const findOne = async (id) => {
    return query("SELECT ProductID, Name, Price from products WHERE ProductID = ?", [id]);
};
...
const updateOne = async (id, product) => {
    return await query("UPDATE products SET ? WHERE ProductID = ?", [product, id]);
};
